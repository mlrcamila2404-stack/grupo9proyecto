  ...
  try {
    ...
    if (data.skills) {
      data.skills.forEach((skill) => {
        const countEl = document.getElementById(skill.tipo + 'Count');
        const percentEl = document.getElementById(skill.tipo + 'Percent');
        const barEl = document.getElementById(skill.tipo + 'Bar');
 
        if (countEl) countEl.textContent = skill.completadas + ' / ' + skill.disponibles;
        if (percentEl) percentEl.textContent = skill.porcentaje + '%';
        if (barEl) barEl.style.width = skill.porcentaje + '%';
      });
    }
