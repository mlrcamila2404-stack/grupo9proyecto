...
async function guardarIntento(idPrueba, elapsedMs = 0) {
  const respuestas = [];
  const base = document.body.dataset.base || '';
  ...
  try {
    const res = await fetch(base + 'intentos/guardar.php', {
      ...
      body: JSON.stringify({ id_prueba: idPrueba, respuestas: respuestas, tiempo: elapsedMs })
    });
...
async function finalizarPrueba(idPrueba) {
  ...
  const resultado = await guardarIntento(idPrueba, elapsedMs);
  ...
  const base = document.body.dataset.base || '';
  window.location.href = base + 'retroalimentacion-excelente.html?id=' + resultado.id_intento + '&score=' + resultado.score + '&correct=' + resultado.correct + '&wrong=' + (resultado.total - resultado.correct) + '&time=' + elapsedMs;
}
